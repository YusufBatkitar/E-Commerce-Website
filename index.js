document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loggedIn) {
        ...
        greetingMessage.textContent = `Hoş geldin, ${storedUser.username}!`;
        greetingMessage.style.display = "block";
        loginBtn.style.display = "none"; 
        signupBtn.style.display = "none";
        logoutBtn.style.display = "inline-block";

        // Kullanıcının ilk girişini kontrol et
        if (!localStorage.getItem("hasVisited")) {
            localStorage.setItem("hasVisited", "true"); // İlk ziyaret olduğunu işaretle
            setTimeout(() => {
                greetingMessage.style.display = "none"; // Mesajı 3 saniye sonra gizle
            }, 3000);
        } else {
            greetingMessage.style.display = "none"; // İlk ziyaret dışında mesajı gizle
        }
    } else {
